...
@onready var object_holder: Marker3D = $Player_Area_Object_Holder
...
func _physics_process(_delta: float) -> void:
	if carried_item != null:
		carried_item.global_position = object_holder.global_position
	_get_area_colliders()
...
func _get_bodies_on_area()->void:
	...
	if array_of_bodies !=null:
		on_area_body = array_of_bodies.back()
...
#el objeto en el area comienza a seguir al marker 3d
func _pick_up_object()->void:
	if(carrying_item):	signal_item_drop.emit()
	else:	signal_item_pickup.emit()
