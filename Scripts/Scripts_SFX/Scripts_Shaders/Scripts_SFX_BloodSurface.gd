...
@export var blood_scale_factor:float = 1.0
...
func _ready():
	...
	if _mesh: 
		...
		blood_scale_factor = max(aabb.size.x,aabb.size.y,aabb.size.z)
	...
	#calculate the size based on the mesh size
	_mat.set_shader_parameter("blood_scale_factor",blood_scale_factor)
