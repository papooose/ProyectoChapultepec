extends state_Base
class_name state_SleepingBeast_Heart_Healthy


signal signal_heart_Healthy
#still recovers health when sleeping

@export_group("State Variables")
@export var max_healt_threshold:int = 200
@export var min_healt_threshold:int = 100
...
func _enter_state()->void:
	...
	signal_heart_Healthy.emit()
	organ_tiroides._change_res(0)
	organ_tiroides.health_condition = 0
	Singleton_Organ_Beast.signal_heart_health_changed.connect(_chekc_health)
	organ_wiring._reset_disconnection_freq(1)
	organ_skin._set_skin_condition(0)
	#change_ anim
	#change_material
...
func _chekc_health()->void:
	if Singleton_Organ_Beast.current_health >= max_healt_threshold:pass
	if Singleton_Organ_Beast.current_health <= min_healt_threshold: FSM._change_state(state_ill)
